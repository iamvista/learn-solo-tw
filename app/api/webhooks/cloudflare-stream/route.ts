...
async function verifyWebhookSignature(
  ...
  // 如果沒有設定 webhook secret，跳過驗證（開發環境）
  if (!webhookSecret) {
    console.warn('警告：未設定 CLOUDFLARE_STREAM_WEBHOOK_SECRET，跳過簽名驗證')
    return true
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!media) {
      console.log(`[Cloudflare Stream Webhook] 找不到對應的 Media 記錄: ${uid}`)
      // 返回 200 以避免 Cloudflare 重試
      return NextResponse.json({ success: true, message: '找不到對應的媒體記錄' })
    }
