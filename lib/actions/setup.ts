...
const setupFormSchema = z.object({
  ...
  // 金流設定
  paymentGateway: z.union([z.enum(['stripe', 'payuni']), z.literal('')]).optional(),
...
export async function completeSetup(
  ...
  try {
    ...
    if (validatedData.siteName) {
      settingsToSave.push({ key: SETTING_KEYS.SITE_NAME, value: validatedData.siteName })
    }
    if (validatedData.contactEmail) {
      settingsToSave.push({ key: SETTING_KEYS.CONTACT_EMAIL, value: validatedData.contactEmail })
    }

    // 金流設定
    const gateway = validatedData.paymentGateway
    if (gateway === 'stripe' || gateway === 'payuni') {
      settingsToSave.push({ key: SETTING_KEYS.PAYMENT_GATEWAY, value: gateway })
    }
    if (validatedData.stripeSecretKey) {
      settingsToSave.push({ key: SETTING_KEYS.STRIPE_SECRET_KEY, value: validatedData.stripeSecretKey })
    }
    if (validatedData.stripeWebhookSecret) {
      settingsToSave.push({ key: SETTING_KEYS.STRIPE_WEBHOOK_SECRET, value: validatedData.stripeWebhookSecret })
    }
    if (validatedData.payuniMerchantId) {
      settingsToSave.push({ key: SETTING_KEYS.PAYUNI_MERCHANT_ID, value: validatedData.payuniMerchantId })
    }
    if (validatedData.payuniHashKey) {
      settingsToSave.push({ key: SETTING_KEYS.PAYUNI_HASH_KEY, value: validatedData.payuniHashKey })
    }
    if (validatedData.payuniHashIV) {
      settingsToSave.push({ key: SETTING_KEYS.PAYUNI_HASH_IV, value: validatedData.payuniHashIV })
    }
    if (validatedData.payuniTestMode !== undefined) {
      settingsToSave.push({ key: SETTING_KEYS.PAYUNI_TEST_MODE, value: String(validatedData.payuniTestMode) })
    }

    // 分析追蹤
    if (validatedData.gaId) {
      settingsToSave.push({ key: SETTING_KEYS.GA_ID, value: validatedData.gaId })
    }
    if (validatedData.metaPixelId) {
      settingsToSave.push({ key: SETTING_KEYS.META_PIXEL_ID, value: validatedData.metaPixelId })
    }
    if (validatedData.metaCapiAccessToken) {
      settingsToSave.push({ key: SETTING_KEYS.META_CAPI_ACCESS_TOKEN, value: validatedData.metaCapiAccessToken })
    }
    ...
    if (error instanceof z.ZodError) {
      return { success: false, error: error.issues[0]?.message || '表單驗證失敗' }
    }
