...
export function SiteSettingsForm({ initialSettings }: SiteSettingsFormProps) {
  ...
  const form = useForm<SiteSettingsFormData>({
    ...
    defaultValues: {
      siteName: initialSettings[SETTING_KEYS.SITE_NAME] || "自由人學院",
          ...
          <TabsContent className="flex flex-col gap-4" value="analytics">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription className="text-[#A3A3A3]">
                        在 Google Analytics 後臺 → 管理 → 資料串流 →
                        網頁串流中找到，格式為 G- 開頭（例如 G-ABC123XYZ）
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription className="text-[#A3A3A3]">
                        在 PostHog 後臺 → Settings → Project → Project API Key
                        中找到，以 phc_ 開頭。這是用來收集前臺用戶行為數據的
                        Key。
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription className="text-[#A3A3A3]">
                        PostHog 的 API 伺服器位址。PostHog Cloud 美國區使用
                        https://us.i.posthog.com，歐洲區使用
                        https://eu.i.posthog.com。如果是自架版本則填入你的伺服器網址。
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription className="text-[#A3A3A3]">
                        在 PostHog 後臺 → 右上角個人頭像 → Settings → Personal
                        API Keys 中建立，以 phx_
                        開頭。這是用來在後臺儀表板查詢分析數據（例如轉換漏斗）的進階功能，不填也可以正常追蹤用戶行為。
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            <Card className="bg-white border border-[#E5E5E5] rounded-xl">
              <CardHeader>
                ...
                <CardDescription className="text-[#525252]">
                  Meta Pixel 用於追蹤網站訪客行為，Conversions API (CAPI)
                  提供伺服器端事件追蹤，提升歸因準確度。
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription className="text-[#A3A3A3]">
                        在 Meta Events Manager → 資料來源 → 你的 Pixel → 設定
                        中找到，為一串 15-16 位數字。
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription className="text-[#A3A3A3]">
                        在 Meta Events Manager → 資料來源 → 你的 Pixel → 設定 →
                        Conversions API →
                        產生存取權杖。用於伺服器端事件追蹤（Purchase、ViewContent），不填則僅使用瀏覽器端
                        Pixel。
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
